...
class Trainer:
    ...
    def train(
        ...
        if args.lr_decay:
            # scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=100, gamma=0.995)
            step = args.lr_decay_factor

            lrs = [args.learning_rate * step ** (n - 1) for n in range(1, epochs+1)]
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for batch, labels in self.train_loader:
                ...
                if ((self.step + 1) % print_frequency) == 0:
                    self.print_metrics(epoch, loss, data_load_time, step_time, cur_lr)
